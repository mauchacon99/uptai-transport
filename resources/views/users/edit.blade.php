                            ...
                            {{ method_field('PUT')}}
                             ...
                             <div class="form-group  @error('name') has-error @enderror">
                                 ...
                                 <div class="group-icon">
                                     <input id="name"   class="form-control" name="name" value="{{ old('name', $user->name ) }}" required autocomplete="name" autofocus>
                             ...
                             <div class="form-group  @error('email') has-error @enderror">
                                 ...
                                 <div class="group-icon">
                                     <input id="email"  value="{{ $user->email }}"  class="form-control" name="email" value="{{ old('email', $user->email ) }}" required autocomplete="email" autofocus>
                                     <span class="icon-envelope text-muted icon-input"></span>
                                     @error('email')
                                 ...
                                 <div class="form-group group-icon @error('role') has-error @enderror">
                                     ...
                                     <div class="group-icon">
                                         ...
                                         @foreach($roles as $rol)
                                           <option value="{{ $rol->name }}"{{ old('rol', $user->roles->first()->id ) == $rol->id ? ' selected' : ''}}> {{ $rol->name }}
                                            <span class="icon-lock text-muted icon-input"></span>
                                           </option>
                                         @endforeach
                                      </select>  
                                      @error('role')
                                        <em id="role" class="error help-block">{{ $message }}</em>
                                      @enderror
                                 </div>
                                 <button type="submit" class="btn btn-block btn-primary btn-rounded box-shadow mt-10"> Editar </button>
                             <hr>
                         </form>
                     </div>
